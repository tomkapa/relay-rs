...
use relay_rs::mcp::oauth::{
    ClientProvenance, McpOAuthClientStore, McpOAuthPendingStore, NewOAuthClient,
...
async fn oauth_client_operator_provenance_overwrites_existing_row() {
    ...
    store
        .upsert(NewOAuthClient {
            org_id: db.default_org_id,
            issuer: "https://issuer.example".into(),
            client_id: "dcr-id".into(),
            client_secret: Some(SecretString::try_from("dcr-secret".to_owned()).expect("valid")),
            authorization_endpoint: "https://issuer.example/auth".into(),
            token_endpoint: "https://issuer.example/token".into(),
            token_endpoint_auth_method: TokenAuthMethod::ClientSecretBasic,
            scope: None,
            provenance: ClientProvenance::Dcr {
    ...
    assert_eq!(replaced.client_id, "operator-id");
    assert_eq!(
